...
QList<GitManager::FileStatus> GitManager::status()
{
    ...
    for (size_t i = 0; i < count; ++i) {
        ...
        if (!e || e->status == GIT_STATUS_CURRENT) continue;

        const unsigned staged_flags = GIT_STATUS_INDEX_NEW
        ...
        if (e->status & staged_flags) {
            ...
            if      (e->status & GIT_STATUS_INDEX_NEW)     s.state = FileStatus::State::Added;
            else if (e->status & GIT_STATUS_INDEX_DELETED)  s.state = FileStatus::State::Deleted;
            else if (e->status & GIT_STATUS_INDEX_RENAMED)  s.state = FileStatus::State::Renamed;
            else                                             s.state = FileStatus::State::Modified;

            if (!s.path.isEmpty()) result.append(s);
        }

        const unsigned wt_flags = GIT_STATUS_WT_MODIFIED
        ...
        if (e->status & wt_flags) {
            ...
            if      (e->status & GIT_STATUS_WT_NEW)        s.state = FileStatus::State::Untracked;
            else if (e->status & GIT_STATUS_WT_DELETED)    s.state = FileStatus::State::Deleted;
            else if (e->status & GIT_STATUS_WT_RENAMED)    s.state = FileStatus::State::Renamed;
            else                                            s.state = FileStatus::State::Modified;
...
bool GitManager::stageFile(const QString &relPath)
{
    ...
    if (err != 0) setError("stage");
    else { git_index_write(index); }
...
bool GitManager::unstageFile(const QString &relPath)
{
    ...
    if (err != 0) { setError("unstage"); return false; }
    emit statusChanged();
    return true;
}
...
bool GitManager::stageAll()
{
    ...
    if (err == 0) git_index_write(index);
    git_index_free(index);

    if (err != 0) { setError("stage all"); return false; }
    emit statusChanged();
    return true;
}
...
bool GitManager::unstageAll()
{
    ...
    if (err != 0) { setError("unstage all"); return false; }
    emit statusChanged();
    return true;
}
...
bool GitManager::discardChanges(const QString &relPath)
{
    ...
    if (err != 0) { setError("discard"); return false; }
    emit statusChanged();
    return true;
}
...
bool GitManager::commit(const QString &message)
{
    ...
    if (git_index_write_tree(&treeOid, index) != 0) {
        setError("write tree"); git_index_free(index); return false;
    }
    git_index_write(index);
    git_index_free(index);
    ...
    if (git_tree_lookup(&tree, repo, &treeOid) != 0) {
        setError("tree lookup"); return false;
    }
    ...
    if (hasParent) git_commit_lookup(&parent, repo, &parentOid);

    git_oid newOid;
    const git_commit *parents[] = { parent };
    const int err = git_commit_create(
        &newOid, repo, "HEAD",
        sig, sig, "UTF-8",
        message.toUtf8(),
        tree,
    ...
    if (err != 0) { setError("commit"); return false; }
    emit statusChanged();
    return true;
}
...
QStringList GitManager::localBranches()
{
    ...
    if (git_branch_iterator_new(&it, repo, GIT_BRANCH_LOCAL) != 0) return result;
...
bool GitManager::checkoutBranch(const QString &name)
{
    ...
    if (git_branch_lookup(&ref, repo, name.toUtf8(), GIT_BRANCH_LOCAL) != 0) {
        setError("branch lookup"); return false;
    }
    ...
    if (err != 0) { setError("checkout"); return false; }
    emit statusChanged();
    return true;
}
...
{
    if (!repo || !hasHead()) return false;

    git_oid headOid;
    if (git_reference_name_to_id(&headOid, repo, "HEAD") != 0) { setError("HEAD"); return false; }
    git_commit *headCommit = nullptr;
    git_commit_lookup(&headCommit, repo, &headOid);
    ...
    git_commit_free(headCommit);
    if (err != 0) { setError("create branch"); return false; }
...
bool GitManager::deleteBranch(const QString &name)
{
    ...
    git_reference_free(ref);
    if (err != 0) { setError("delete branch"); return false; }
    emit statusChanged();
    return true;
}
...
QList<GitManager::CommitInfo> GitManager::log(int limit)
{
    ...
    if (!repo || !hasHead()) return result;

    git_revwalk *walk = nullptr;
    if (git_revwalk_new(&walk, repo) != 0) return result;
    ...
    for (int i = 0; i < limit && git_revwalk_next(&oid, walk) == 0; ++i) {
        ...
        if (git_commit_lookup(&c, repo, &oid) != 0) continue;
        ...
        info.date = author
            ? QDateTime::fromSecsSinceEpoch(author->when.time).toString("yyyy-MM-dd HH:mm")
            : QString{};
...
QString GitManager::commitDiff(const QString &fullHash)
{
    ...
    git_oid oid;
    if (git_oid_fromstr(&oid, fullHash.toUtf8()) != 0) return {};
